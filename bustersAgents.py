    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        likelyGhostPos = [b.argMax() for b in livingGhostPositionDistributions]

        # find closest ghost
        closestPos = min(likelyGhostPos, key=lambda pos: self.distancer.getDistance(pacmanPosition, pos))

        # choose the best action that minimizes dis
        bestAct = min(legal, key=lambda act: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, act), closestPos))
