...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            vkplus1 = util.Counter()
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state):
                    vkplus1[state] = 0.0
                else:
                    vkplus1[state] = max(
                        map(lambda action: self.computeQValueFromValues(state, action), 
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        return sum(
            map(lambda sp: sp[1] * (self.mdp.getReward(state, action, sp[0]) + self.discount * self.values[sp[0]]),
